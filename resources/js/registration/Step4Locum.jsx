...
function Step4Locum({ user, setUser, setStep }) {
    useLayoutEffect(() => {
        if (!user?.profession || !user?.role) return;
        ...
        axios
            ...
            .then((res) => {
                ...
                allSelectFields.forEach((element) => {
                    ...
                    if (user?.questions_answers?.[questionId]) {
                ...
                allCheckboxes.forEach((element) => {
                    ...
                    if (user?.questions_answers?.[questionId]) {
                        ...
                        if (Array.isArray(arr) && arr.includes(element.value)) {
            ...
            <div>
                <form action={REGISTER_FORM_URI} method="post" id="my-submit-form">
                    <div style={{ display: "none" }}>
                        ...
                        {/* Store List Fix */}
                        {Array.isArray(user?.site_town_ids) &&
                            user?.site_town_ids?.map((town_id) => (
                                <input key={town_id} type="hidden" name="store_list[]" value={town_id} />
                            ))}
                    ...
                    <div className="col-md-12 btn-img paypalbtnn">
                        <button type="button" className="btn btn-default btn-1 lkbtn" style={{ marginRight: "8px" }} onClick={() => setStep(3)}>
                            <span><i className="fa fa-angle-double-left"></i> Back</span>
                        </button>
